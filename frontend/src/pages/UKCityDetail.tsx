...
const UKCityDetail: React.FC = () => {
    ...
    useEffect(() => {
        const fetchRestaurants = async () => {
            if (!cityNameFromSlug) return;

            try {
                setLoading(true);
                const response = await fetch(
                    `${config.apiUrl}/api/restaurants/location?city=${cityNameFromSlug}&country=UK`
                );
                if (!response.ok) {
        ...
        <div>
            <Breadcrumbs
                customItems={[
                    ...
                    { name: cityName, path: `/uk/city/${citySlug}` },
                ]}
            />
